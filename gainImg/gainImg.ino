...
//Определение пина и размеров светодиодной ленты
#define LED_PIN 4
...
//Массивы содержищие значения цветов пикселей светодиодки (в двумерном и в прямом виде)
CRGB screen[Width][Height]; 
CRGB screen1D[Width*Height];

//Функция вывода на матрицу
void Draw(CRGB from2D[x][y])
{
	//выпрямление двумерного массива в прямую линию (в реальности она сложена змейкой)
	int k = 0;
...
//Переменные отвеающие за то, выводиться ли сейчас анимация ожидания и переменная кадра анимации
bool IsStart = true;
int anim = 0;

//Анимация ожидания подключенных устройств
void Anim_WaitWiFi() 
{
	if(anim == 0)
	{
		uint8_t ArrayBmp[Width][Height][3] =
		{
		...
		{{0,142,205},{0,142,205},{0,142,205},{0,142,205},{0,142,205},{0,142,205},{0,142,205},{0,142,205},{0,142,205},{0,142,205},{0,142,205},{0,142,205}},
		{{0,142,205},{0,142,205},{0,142,205},{0,142,205},{0,142,205},{0,142,205},{0,142,205},{0,142,205},{0,142,205},{0,142,205},{0,142,205},{0,142,205}}
		};//картинка ожидания подключения к WIFI
		for(int i = 0; i < Width; i++)
		{
			for(int j = 0; j < Height; j++)
			{
				screen[i][j] = CRGB(ArrayBmp[i][j][0],ArrayBmp[i][j][1],ArrayBmp[i][j][2]);
			}
		}
	}
	if(anim == 1)
...
//Функция отображения картинки загрузки
void Anim_Loading()
{
...
//Функция преобразования текста в массив цифр(строковых)
...
//Функция принимающая изображение
void Download()
{
	IsStart = false;
...
void setup()
{
	FastLED.addLeds<WS2811, LED_PIN, GRB>(screen1D, Width*Height).setCorrection(TypicalLEDStrip);
